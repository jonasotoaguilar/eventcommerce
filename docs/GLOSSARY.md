...
| Operator | The privileged role allowed alongside the owner on commerce reads and required alone on `POST /api/v1/orders/{order_id}/confirm` and `POST /api/v1/orders/{order_id}/cancel` (`backend/app/modules/orders/api/routes.py`). | Now |
...
- `checkout` — synchronous commerce orchestrator (`POST /api/v1/checkout`; inline items or `cart_id` with catalog-derived pricing; retained compatibility boundary alongside the async path).
...
**Target contexts** (MVP): no new bounded contexts remain; the five-state `orders` lifecycle is delivered (see Order status).

## Events

The shared envelope at `backend/app/shared/messaging/envelope.py` defines the canonical `event_type` literal: `OrderCreated`, `InventoryReserved`, `InventoryRejected`, `OrderInventoryReserved`, `PaymentAuthorized`, `PaymentRejected`, `OrderPaymentAuthorized`, `OrderConfirmed`, and `OrderCancelled`. The shared event store (`backend/app/shared/events/`) persists timeline events; the transactional outbox (`backend/app/shared/messaging/outbox_repository.py`) persists events for later forwarding. Only the `orders` context defines domain event dataclasses today (`backend/app/modules/orders/domain/events.py`); inventory, payments, and notifications have no per-module events module.
...
| `OrderConfirmed` | The order reached the `confirmed` terminal state. | `orders` (via `FinalizePaymentAuthorized` / `ConfirmOrder`) and `checkout` (synchronous path) | `ProcessOrderNotification` via `notifications.order_terminal` (plus best-effort sync notify) | Outbox writes in `backend/app/modules/orders/application/finalize_payment_authorized.py`, `backend/app/modules/orders/application/confirm_order.py`, and `backend/app/modules/checkout/application/checkout.py` |
| `OrderCancelled` | The order reached the `cancelled` terminal state. | `orders` (via `ProcessOrderInventoryResult` / `ProcessOrderPaymentResult` / `CancelOrder`) and `checkout` (synchronous path) | `ProcessOrderNotification` via `notifications.order_terminal`; inventory release (on payment failure) | Outbox writes in `backend/app/modules/orders/application/process_inventory_result.py`, `backend/app/modules/orders/application/process_payment_result.py`, `backend/app/modules/orders/application/cancel_order.py`, and `backend/app/modules/checkout/application/checkout.py` |
| `InventoryReserved` | Requested stock was reserved successfully. | `inventory` (via `ProcessInventoryReservation`) | `ProcessOrderInventoryResult` via `orders.inventory_result` | Envelope literal; `orders/domain/events.py` dataclass |
| `InventoryRejected` | Requested stock could not be reserved. | `inventory` (via `ProcessInventoryReservation`) | `ProcessOrderInventoryResult` via `orders.inventory_result` | Envelope literal; `orders/domain/events.py` dataclass |
| `OrderInventoryReserved` | Internal order-owned event staging payment authorization after the durable `inventory_reserved` transition. | `orders` (via `ProcessOrderInventoryResult`) | `ProcessOrderInventoryReserved` via `payments.inventory_reserved` | `backend/app/modules/orders/domain/events.py` |
| `PaymentAuthorized` | Payment was authorized against authoritative catalog totals. | `payments` (via `ProcessOrderInventoryReserved`) | `ProcessOrderPaymentResult` via `orders.payment_result` | `backend/app/modules/payments/application/process_inventory_reserved.py` |
| `PaymentRejected` | Payment authorization failed (catalog or provider reason). | `payments` (via `ProcessOrderInventoryReserved`) | `ProcessOrderPaymentResult` + inventory compensation via `orders.payment_result` | `backend/app/modules/payments/application/process_inventory_reserved.py` |
| `OrderPaymentAuthorized` | Internal order-owned event staging confirmation after the durable `payment_authorized` transition. | `orders` (via `ProcessOrderPaymentResult`) | `FinalizePaymentAuthorized` via `orders.payment_authorized` | `backend/app/modules/orders/domain/events.py` |

### Consumer wiring (Now)

The choreography runtime wires these events to consumers through the outbox, RabbitMQ publisher, and AMQP consumer (`backend/app/messaging_runtime.py`, durable `order.events` TOPIC exchange): `inventory.order_created` carries `OrderCreated` → `ProcessInventoryReservation`; `orders.inventory_result` carries `InventoryReserved`/`InventoryRejected` → `ProcessOrderInventoryResult`; `payments.inventory_reserved` carries `OrderInventoryReserved` → `ProcessOrderInventoryReserved` (catalog-derived authorization); `orders.payment_result` carries `PaymentAuthorized`/`PaymentRejected` → `ProcessOrderPaymentResult` (+ inventory compensation on rejection); `orders.payment_authorized` carries `OrderPaymentAuthorized` → `FinalizePaymentAuthorized`; `notifications.order_terminal` carries `OrderConfirmed`/`OrderCancelled` → `ProcessOrderNotification`. The synchronous checkout path is retained as the compatibility boundary alongside the wired consumers. Live broker delivery requires RabbitMQ (gated CI integration); the default suite proves the chain broker-free (`backend/app/tests/runtime/test_chain_e2e.py`).

## State vocabulary

### Order status

Order statuses are `pending`, `inventory_reserved`, `payment_authorized`, `confirmed`, and `cancelled`. The allowed transitions are defined by `can_transition` in `backend/app/modules/orders/domain/services.py`: the event-driven path walks `pending` → `inventory_reserved` → `payment_authorized` → `confirmed`/`cancelled`, while the synchronous checkout keeps the `pending` → `confirmed` compatibility shortcut.
...
| `payment_authorized` | `payment_authorized` | Yes | Idempotent self-transition. |
| `payment_authorized` | `confirmed` | Yes | Reached by the payment-authorized finalizer or the operator confirm route. |
| `payment_authorized` | `cancelled` | Yes | Operator cancel. |
...
2. The **Current events** table reflects the shared envelope `event_type` literal in `backend/app/shared/messaging/envelope.py`, the domain event dataclasses in `backend/app/modules/orders/domain/events.py`, and the outbox emissions from the checkout path and the lifecycle consumers. Keep it in sync with those files.
